...
//		));
	...
	public function authFacebook(Request $request, Response $response) {
		...
		try {
			...
			// check if the user has been logged
			if (!$fb_session) {
				return $this->responseJsonError('no_user_account', 'El usuario no se encuentra logueado.');
			}
			...
			// @todo: log real message: $ex->getMessage()
			return $this->responseJsonError('app_error', 'Se ha producido un error al leer la sesión del usuario.');
		}
	}
